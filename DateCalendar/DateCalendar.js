var DateCalendar = /** @class */ (function () {
    ...
    DateCalendar.prototype.renderDom = function () {
        //初始化渲染控件DOM
        var i = 0, size = 3, self = this, dateTitle = '<table class="date-table-title"><thead><tr>' +
            '<th class="weekend">日</th><th>一</th><th>二</th>' +
            '<th>三</th><th>四</th><th>五</th>' +
            '<th class="weekend">六</th></tr></thead></table>', wrapper = self.opts.tar, docFragment = document.createDocumentFragment(), oDateCalendarWrapper = document.createElement('div'), oDateCalendarContent;
    ...
    DateCalendar.prototype.renderTable = function () {
        //初始化渲染表格
        var i = 0, size = 42, self = this, docFragment = document.createDocumentFragment(), oTable = document.createElement('table');
    ...
    DateCalendar.prototype.fillTable = function (year, month, dateCalendarId) {
        //向表格填充日期
        var self = this, calendarDom = document.getElementById(dateCalendarId), oTableList = calendarDom.getElementsByTagName('table'), prevYearMonth = self.getPrevMonth(year, month), nextYearMonth = self.getNextMonth(year, month);
    ...
    DateCalendar.prototype.freshTds = function (tableDom, year, month) {
        //刷新表格日期数据
        var i = 0, i_ = 0, self = this, oTable = tableDom, rowSum = self.getRowSum(year, month), monthArr = self.initMonthArr(year, month), oTrList = tableDom.getElementsByTagName('tr'), oTdList = tableDom.getElementsByTagName('td'), arrLen = monthArr.length, trLen = oTrList.length;
    ...
    DateCalendar.prototype.initOneLine = function () {
        var i = 0, self = this, oLayer = document.getElementById('date-calendar-content-1'), 
        // oTable = oLayer.children[1],
        oTable = oLayer.getElementsByTagName('table')[1], oTrs = oTable.getElementsByTagName('tr'), trLen = oTrs.length, rowNum = this.getRowNumberOfToday(), outerWrapper = this.opts.tar;
    ...
    DateCalendar.prototype.getRowNumberOfToday = function () {
        //计算今天在控件第几行 从0开始
        var day = this.opts.todayDate, month = this.opts.todayMonth, year = this.opts.todayYear, beginBlank = this.getBeginBlankSum(year, month), num = day + beginBlank - 1;
        return Math.floor(num / 7);
    };
    DateCalendar.prototype.posiCenter = function () {
        //将3个层级相对屏幕水平居中显示
        var self = this, layer1 = document.getElementById('date-calendar-content-1'), layer2 = document.getElementById('date-calendar-content-2'), layer3 = document.getElementById('date-calendar-content-3');
    ...
    DateCalendar.prototype.pushToTop = function (num) {
        //置顶显示对应层
        var top = '9999', bottom = '8888', layer1 = document.getElementById('date-calendar-content-1'), layer2 = document.getElementById('date-calendar-content-2'), layer3 = document.getElementById('date-calendar-content-3');
        if (num === 2) {
    ...
    DateCalendar.prototype.initMonthArr = function (year, month) {
        //初始化某月的数组数据
        ...
        for (; i <= days; i++) {
            var obj = {
                ...
                'isWorkday': self.isWorkday(year, month, i),
    ...
    DateCalendar.prototype.isWorkday = function (year, month, day) {
        //判断某天是否是工作日 return true or false
        var i = 0, self = this, arrLen, workdayArr = self.opts.workdayArr, year = year, month = self.holdByZero(month), day = self.holdByZero(day), dateStr = year + '-' + month + '-' + day;
    ...
    DateCalendar.prototype.bindEvent = function (selector, type, callback) {
    ...
    DateCalendar.prototype.unbindEvent = function (selector, type, callback) {
        //解绑事件处理函数
        var tarDom;
        if (typeof selector === 'string') {
            tarDom = document.getElementById(selector);
    ...
    DateCalendar.prototype._touchCallback = function (event) {
        //touch事件分发
    ...
    DateCalendar.prototype._touchstartCallback = function (event) {
    ...
    DateCalendar.prototype._touchmoveCallback = function (event) {
        var self = this;
    ...
    DateCalendar.prototype._touchmoveHandler = function (event) {
        var i = 0, self = this, oLayer = document.getElementById('date-calendar-content-1'), oTable = oLayer.children[1], oTrs = oTable.getElementsByTagName('tr'), trLen = oTrs.length, outerWrapper = this.opts.tar, movingHeight = self.opts.beginY + self.opts.offsetY, rowNum = this.getRowNumberOfToday();
    ...
    DateCalendar.prototype._touchendHandler = function (event) {
        var i = 0, self = this, oWrapper = document.getElementById('date-calendar-wrapper'), oLayer = document.getElementById('date-calendar-content-1'), 
        // oTable = oLayer.children[1],
        oTable = oLayer.getElementsByTagName('table')[1], oTrs = oTable.getElementsByTagName('tr'), trLen = oTrs.length, outerWrapper = this.opts.tar;
    ...
    DateCalendar.prototype._touchendCallback = function (event) {
    ...
    DateCalendar.prototype.animatingSlide = function (flag) {
        //动画效果
        var self = this, cachedYearMonth, rest = 100;
    ...
    DateCalendar.prototype._callback_ = function (event) {
        //私有方法 分发事件委托到处理函数
    ...
    DateCalendar.prototype._clickCallback = function (target, data) {
        //点击日期单元格的 内部处理函数 外部的在参数里
        console.info(target, data);
        var i = 0, oTable = target.parentNode.parentNode.parentNode, tdList = oTable.getElementsByTagName('td'), length = tdList.length;
        for (; i < length; i++) {
    ...
    DateCalendar.prototype.getWeekOfFirstDay = function (year, month) {
        //某年某月的第一天星期几
        return new Date(year, month - 1, 1).getDay();
    };
    DateCalendar.prototype.getWeekOfLastDay = function (year, month) {
        //某年某月的最后一天星期几
        return new Date(year, month - 1, this.getDayPlainSumOfMonth(year, month)).getDay();
    };
    DateCalendar.prototype.getBeginBlankSum = function (year, month) {
        //获取某年某月第一天前的空白数
        return this.getWeekOfFirstDay(year, month);
    };
    DateCalendar.prototype.getEndBlankSum = function (year, month) {
        //获取某年某月最后一天后的空白数
        return 6 - this.getWeekOfLastDay(year, month);
    };
    DateCalendar.prototype.getDayPlainSumOfMonth = function (year, month) {
        //获取某月的实际天数
        if (month === 2) {
    ...
    DateCalendar.prototype.getDayFormatedSumOfMonth = function (year, month) {
    ...
    DateCalendar.prototype.getRowSum = function (year, month) {
        //某年某月占控件几行 至少占4行 最多占6行
        return this.getDayFormatedSumOfMonth(year, month) / 7;
    };
    DateCalendar.prototype.getPrevMonth = function (year, month) {
        //获取当前月的上一月 当前月如果是1月 则上一月为上一年的12月
        return month === 1 ? [year - 1, 12] : [year, month - 1];
    };
    DateCalendar.prototype.getNextMonth = function (year, month) {
        //获取当前月的下一月 当前月如果是12月 则下一月为下一年的1月
        return month === 12 ? [year + 1, 1] : [year, month + 1];
    };
    DateCalendar.prototype.getWeek = function (year, month, day) {
        //获取某天是星期几
        return new Date(year, month - 1, day).getDay();
    };
    DateCalendar.prototype.isLeapYear = function (year) {
        //判断是否是闰年
        return (year % 4 === 0 && year % 100 !== 0) || year % 400 === 0 ? true : false;
    };
    DateCalendar.prototype.isToday = function (year, month, day) {
        //判断某天是否是今天
        return !!(year === this.opts.todayYear && month === this.opts.todayMonth && day === this.opts.todayDate);
    };
    DateCalendar.prototype.isWeekend = function (year, month, day) {
        //判断某天是否是周末
        var week = new Date(year, month - 1, day).getDay();
        return (week === 0 || week === 6) ? true : false;
    };
    DateCalendar.prototype.isSunday = function (year, month, day) {
        //判断某天是否是周日
        var week = new Date(year, month - 1, day).getDay();
        return week === 0 ? true : false;
    };
    DateCalendar.prototype.holdByZero = function (num) {
        return num > 9 ? num : '0' + num;
    };
    DateCalendar.prototype.addClass = function (selector, classes) {
        //为dom元素新增类  selector为 id 或者 dom对象
        if (this.hasClass(selector, classes)) {
            return;
        }
        var dom = typeof selector === 'string' ? document.getElementById(selector) : selector, classArr = dom.className.split(/\s+/);
        classArr[classArr.length] = classes;
        dom.className = classArr.join(' ');
    };
    DateCalendar.prototype.removeClass = function (selector, classes) {
        //为dom元素删除特定类  selector为 id 或者 dom对象
        if (!this.hasClass(selector, classes)) {
            return;
        }
        var dom = typeof selector === 'string' ? document.getElementById(selector) : selector, classStr = ' ' + dom.className.split(/\s+/).join(' ') + ' ', removingClass = ' ' + classes + ' ';
        dom.className = classStr.replace(removingClass, '').replace(/^\s+|\s+$/g, '');
    };
    DateCalendar.prototype.hasClass = function (selector, classes) {
        var dom = typeof selector === 'string' ? document.getElementById(selector) : selector, classArr = dom.className.split(/\s+/);
        return !!~classArr.indexOf(classes);
    };
    DateCalendar.prototype.merge = function (obj, _obj) {
        //合并参数对象
